...
__export(index_exports, {
  CardanoQueryClient: () => QueryClient,
  CardanoSubmitClient: () => SubmitClient,
  CardanoSyncClient: () => SyncClient,
  CardanoWatchClient: () => WatchClient
});
...
var createGrpcTransport = import_connect_node.createGrpcTransport;
...
function pointToBlockRef(p) {
  return new import_spec.sync.BlockRef({
    index: BigInt(p.slot),
    hash: new Uint8Array(import_buffer.Buffer.from(p.hash, "hex"))
  });
}
function blockRefToPoint(r) {
  return {
    slot: r.index.toString(),
    hash: import_buffer.Buffer.from(r.hash).toString("hex")
  };
}
...
var SyncClient = class {
  constructor(options) {
    ...
    this.inner = (0, import_connect.createPromiseClient)(import_spec.syncConnect.SyncService, transport);
  }
  async *followTip(intersect) {
    const req = new import_spec.sync.FollowTipRequest({
  ...
  async fetchHistory(p, maxItems = 1) {
    const req = new import_spec.sync.DumpHistoryRequest({
      startToken: new import_spec.sync.BlockRef({
        index: BigInt(p.slot),
        hash: import_buffer.Buffer.from(p.hash, "hex")
      }),
      maxItems
    });
...
var QueryClient = class {
  constructor(options) {
    ...
    this.inner = (0, import_connect.createPromiseClient)(import_spec.queryConnect.QueryService, transport);
  }
  async readParams() {
...
var SubmitClient = class {
  constructor(options) {
    ...
    this.inner = (0, import_connect.createPromiseClient)(import_spec.submitConnect.SubmitService, transport);
  }
  async submitTx(tx) {
...
var WatchClient = class {
  constructor(options) {
    ...
    this.inner = (0, import_connect.createPromiseClient)(import_spec.watchConnect.WatchService, transport);
  }
  async *watchTxByMatch(pattern, intersect) {
    const request = new import_spec.watch.WatchTxRequest({
