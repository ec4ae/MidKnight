...
var U5CProvider = class {
   ...
   */
  async evaluateTx(tx) {
    const report = await this.submitClient.evalTx((0, import_common6.hexToBytes)(tx));
   ...
   */
  async fetchAddressUTxOs(address, asset) {
    const addressBytes = (0, import_common6.hexToBytes)(import_core_cst4.Address.fromBech32(address).toBytes());
  ...
  async fetchUTxOs(hash, index) {
    const utxoSearchResult = await this.queryClient.readUtxosByOutputRef(
      [{
        txHash: (0, import_common6.hexToBytes)(hash),
        outputIndex: index || 0
